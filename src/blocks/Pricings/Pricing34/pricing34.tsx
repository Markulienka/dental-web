    ...
    <section className={`py-24 ${backgroundMuted ? 'bg-muted' : ''}`}>
      <div className="container flex flex-col gap-12">
        <div className="mx-auto max-w-2xl text-center">
          <h1 className="mb-4 text-4xl font-bold tracking-tight text-primary lg:text-5xl">
            {title}
          </h1>
        ...
        <div className="flex flex-wrap justify-center gap-7 items-stretch">
          {card.map((plan, index) => (
            <div key={index} className="relative flex w-full max-w-sm">
              ...
              <Card
                className={`w-full rounded-2xl border flex flex-col ${plan.highlighted
                  ? 'border-primary border-2 bg-primary-foreground'
                  : 'border-border bg-primary-foreground'
                  } shadow-sm`}
              >
                <CardHeader className={centerAlign ? 'text-center' : ''}>
                  <CardTitle
                    className={`text-primary text-lg font-medium ${centerAlign ? 'text-center' : ''}`}
                  >
                    {plan.name}
                  </CardTitle>
                  <div
                  ...
                  >
                    ...
                    <div className="text-foreground text-sm">/mesiac</div>
                  </div>
                  {plan.description && <p className="mt-2 text-sm text-foreground">{plan.description}</p>}
                </CardHeader>
                ...
                <CardContent className="px-7 pt-6 flex-1 flex flex-col">
                  ...
                  <ul className="mt-6 space-y-3 flex-1">
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className="flex items-center gap-3">
                        ...
                        <span
                          className={`text-primary text-sm ${feature.bold ? 'font-bold' : 'font-medium'}`}
                        >
                          {feature.text}
                        </span>
                      </li>
                  ...
                  {plan.linkCard && (
                    <CMSLink
                      ...
                      appearance="default"
                      className={`mt-6 inline-flex h-10 w-full items-center justify-center rounded-lg text-sm font-medium transition-colors ${plan.highlighted
                        ? 'bg-primary text-primary-foreground hover:bg-primary/90'
                        : 'border border-border bg-primary-foreground text-primary hover:bg-primary-foreground/80'
                        }`}
                    />
                  )}
                </CardContent>
